...
def regex_find(reg_expresssion:str, tagged_words:list)->list:
    ...
    for subtree in chunked_words.subtrees():
        ...
        if subtree.label() == "Conjunction":
            print(subtree)
            for i in subtree:
                if i == ("and", "CC") or i == ("or", "CC") or i == ("but", "CC"):
        ...
        if subtree.label() == "Suborinary_conj":
            for subconj in subtree:
                if subconj == ("if", "IN") or subconj == ("because", "IN") or subconj == ("whether", "IN"):
...
def find_cut_pos(tokenized_sentences:str)->list:
    ...
    split_regex = r"""Conjunction: {<RB.*?|JJ.*?|VB.*?|IN?|MD?>?<,><CC><DT>?<RB.?|VBZ?|JJ.|IN>*<PRP>?}
...
def merge_sentence(sentences:list, transcribe_res:list):
    """
